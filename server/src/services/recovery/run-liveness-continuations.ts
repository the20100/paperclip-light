...
// Empty adapter responses may be transient and get a bounded retry. Narrated
// future work is deliberately excluded: a manager/recovery owner must decide
// the next action instead of paying the same agent to restate its plan.
const ACTIONABLE_LIVENESS_STATES = new Set<RunLivenessState>(["empty_response"]);
